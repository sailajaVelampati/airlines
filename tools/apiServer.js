...
server.post("/passenger/", function (req, res, next) {
  const error = validateCourse(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.seatNo); // Generate a slug for new passenger.
    next();
  }
});
...
function validateCourse(passenger) {
  if (!passenger.seatNo) return "seatNo is required.";
